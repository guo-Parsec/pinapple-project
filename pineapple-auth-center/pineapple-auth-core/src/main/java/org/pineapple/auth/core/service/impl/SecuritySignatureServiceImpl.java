...
public class SecuritySignatureServiceImpl implements SecuritySignatureService {
    ...
    @Autowired
    public void setDefaultCryptoStrategy(@Qualifier("md5SaltCrypto") CryptoStrategy defaultCryptoStrategy) {
        this.defaultCryptoStrategy = defaultCryptoStrategy;
    }
     ...
     * <p>获取密码加解密策略器</p>
     *
     * @return {@link org.pineapple.common.strategy.CryptoStrategy }
     * @author guocq
     * @date 2023/3/16 14:53
     */
    private CryptoStrategy findCryptoStrategy() {
        ...
        if (systemParamEntity == null || StrUtil.isBlank(systemParamEntity.getParamValue())) {
            ...
            return this.defaultCryptoStrategy;
        }
    ...
    public SecuritySignature loadSignature(String loginId, String rawPassword) {
        ...
        CryptoStrategy cryptoStrategy = this.findCryptoStrategy();
        Map<String, Object> optionMap = Maps.newHashMap();
        optionMap.put(Md5SaltCrypto.OPTION_KEY_SALT, loginId);
        if (!cryptoStrategy.match(rawPassword, sysUserVo.getLoginPassword(), optionMap)) {
